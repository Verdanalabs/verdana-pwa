...
import { listMaggotBatches, type MaggotBatch } from '@/src/features/maggot/services/maggot-api';

export default function MaggotListScreen() {
  const c = useThemeColors();
  const { token } = usePvpAuth();
  const [batches, setBatches] = useState<MaggotBatch[]>([]);
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: c.background }]} edges={['top']}>
      <View style={styles.topBar}>
        ...
        <Text style={[styles.topTitle, { color: c.foreground }]}>Maggot Farming</Text>
        <View style={{ width: 42 }} />
      </View>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.center}>
              <Ionicons name="bug-outline" size={36} color={c.textMuted} />
              <Text style={[styles.muted, { color: c.textMuted }]}>No maggot batches yet.</Text>
            </View>
          }
          renderItem={({ item }) => (
            <TouchableOpacity
              style={[styles.card, { backgroundColor: c.surface, borderColor: c.border }]}
              onPress={() => router.push(`/maggot/${item.id}`)}
              activeOpacity={0.8}
            >
              <View style={styles.cardRow}>
                <Text style={[styles.cardTitle, { color: c.foreground }]}>
                  {(item.organic_weight_grams / 1000).toFixed(1)} kg organic
                </Text>
                <View style={[styles.pill, { backgroundColor: item.status === 'completed' ? `${c.accent}18` : '#f59e0b20' }]}>
                  <Text style={[styles.pillText, { color: item.status === 'completed' ? c.accent : '#f59e0b' }]}>
                    {item.status === 'completed' ? 'harvested' : 'feeding'}
                  </Text>
                </View>
              </View>
              {item.yield_percent != null && (
                <Text style={[styles.cardSub, { color: c.textMuted }]}>Yield {item.yield_percent}%</Text>
              )}
            </TouchableOpacity>
          )}
        />
      ...
      <View style={[styles.footer, { borderTopColor: c.border, backgroundColor: c.background }]}>
        <TouchableOpacity
          ...
          onPress={() => router.push('/maggot/new')}
          activeOpacity={0.85}
        >
          <Ionicons name="add" size={20} color={c.accentContrast} />
          <Text style={[styles.primaryBtnLabel, { color: c.accentContrast }]}>New Maggot Batch</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
