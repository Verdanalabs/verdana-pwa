...
export default function PvpDashboardTab() {
  ...
  const arriving = MOCK_BATCHES.filter((b) => b.status === 'transit');
  const readyToInspect = MOCK_BATCHES.filter((b) => b.status === 'pending_validation');
  const doneToday = MOCK_BATCHES.filter((b) => b.status === 'minted' || b.status === 'verified');
                    ...
                    <View style={styles.queueTop}>
                      <View>
                        ...
                        <Text style={[styles.queueTime, { color: c.textMuted }]}>
                          {timeLabel(batch.submittedAt)} · {timeAgo(batch.submittedAt)} ago
                        </Text>
                      </View>
                      {isUrgent && (
