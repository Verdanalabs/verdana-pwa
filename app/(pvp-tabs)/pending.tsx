...
const MOCK_COSIGN_BATCH = MOCK_BATCHES.find((b) => b.status === 'transit') ?? MOCK_BATCHES[0];
      ...
      <SafeAreaView style={[styles.safe, { backgroundColor: c.background }]}>
        <View style={styles.centerState}>
          ...
          <Text style={[styles.stateSub, { color: c.textMuted }]}>
            The co-sign session for {MOCK_COSIGN_BATCH.id} was cancelled.
          </Text>
          <TouchableOpacity
    ...
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: c.background }]}>
        <View style={styles.centerState}>
          ...
          <Text style={[styles.stateSub, { color: c.textMuted }]}>
            Batch {MOCK_COSIGN_BATCH.id} has been co-signed. Minting job queued.
          </Text>
          <TouchableOpacity
            style={[styles.resetBtn, { borderColor: c.border }]}
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <Text style={[styles.pageSub, { color: c.textSecondary }]}>
            {MOCK_COSIGN_BATCH.id} · Supplier notified
          </Text>
        </View>
        ...
        <View style={[styles.detailCard, { backgroundColor: c.surface, borderColor: c.border }]}>
          {[
            { label: 'SUPPLIER', value: `Supplier #${MOCK_COSIGN_BATCH.id}` },
            { label: 'BATCH', value: MOCK_COSIGN_BATCH.id },
            { label: 'ACTUAL WEIGHT', value: `${MOCK_COSIGN_BATCH.actualWeightKg ?? MOCK_COSIGN_BATCH.estimatedWeightKg} Kg` },
            { label: 'MATERIAL', value: `${MOCK_COSIGN_BATCH.materialType} plastic` },
            { label: 'PVP ID', value: operator?.stationId ?? '—' },
            { label: 'SENT AT', value: timeLabel(MOCK_COSIGN_BATCH.submittedAt ?? '') },
          ].map((row, i, arr) => (
            <View
              key={row.label}
