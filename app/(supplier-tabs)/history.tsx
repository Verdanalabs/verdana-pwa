...
export default function HistoryRoute() {
  ...
  const filteredBatches = useMemo(() => (
    MOCK_BATCH_SUMMARIES.filter((batch) => {
      const fullBatch = MOCK_BATCHES.find((item) => item.id === batch.id);
  ...
  const totals = useMemo(() => ({
    ...
    active: MOCK_BATCHES.filter((batch) =>
      ['submitted', 'transit', 'pending_validation', 'verified', 'minting'].includes(batch.status)
    ).length,
    ready: MOCK_BATCHES.filter((batch) => batch.status === 'minted').length,
  }), []);
